...
import { faHome, faBars, faSearch, faXmark, faUser, faYinYang } from '@fortawesome/free-solid-svg-icons';
...
const Navbar = ({ setProps }) => {
  ...
  const navLinks = menuItems.map((item, index) => {
    return (
      <div className="flex justify-between items-center" key={index}>
        <a href={item.href} className='font-medium hover:text-white'>
  ...
  return (
    <nav className='p-3 flex justify-between items-center bg-primary-nav text-gray-300'>
      ...
      <div id='nav-dialog' className='hidden fixed inset-0 bg-primary-nav text-gray-300 p-3'>
        <div id="nav-bar" className='flex justify-between'>
          <a href="/" className='flex items-center gap-2'>
            <img src={logo} alt="logo" className='md:size-20 xs:size-16' />
            <span className='text-2xl font-medium italic' id="brand-name">MusiX</span>
          </a>
          <button onClick={handleMenu} className='md:hidden'>
            <FontAwesomeIcon icon={faXmark} className='text-3xl' id='menu-bar' />
          </button>
        </div>
        ...
        <div className="flex flex-col items-center  ">
          ...
          <button className=" bg-blue-700 font-bold rounded-md px-3 py-1 mt-2 w-full" onClick={handleSearch}>
            Search
          </button>
        </div>
      </div>
